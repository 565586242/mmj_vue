<template>
	<div>
		...
		<label for class="add_mr">
			<input type="checkbox" v-bind:true-value="2" v-bind:false-value="1" class="t_romm_ip" v-model="checkedValue" />设为默认地址
		</label>
		<button class="sign_out" @click="bc_btn()">保存地址</button>
	...
	export default {
		...
		methods: {
			...
			bc_btn() {
				...
				mui.ajax(config.host + "/addressdoadd ", {
					data: {
						user_name: _this.user,
						user_phone: _this.phone,
						user_address: _this.addressChangeRes,
						user_street: _this.address,
						is_default: _this.checkedValue
					},
