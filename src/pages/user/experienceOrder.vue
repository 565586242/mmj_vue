<template>
	<div class="shop_order">
		<div class="change_password_box">
			<ul class="tabs">
				<li class="li-tab" v-for="(item,index) in tabsParam" :key="index" @click="toggleTabs(index)" :class="{active:index==noworder}">
					<span>{{item}}</span>
				</li>
			</ul>
			...
			<div class="divTab">
				<div class="order_main_box" v-for="(item,index) in shop_order" :key="index">
					<div class="order_main">
						...
						<router-link :to="{path:'/experienceOrderDetail',query:{ id:item.id}}">
					...
					<div class="order_moner">
						<p>
							<button style="position: absolute;margin-left: -2.2rem;" v-show="item.status=='已完成'" @click="deleteOrder(item.id)">删除订单</button>
							<button style="position: absolute;" v-show="item.status=='待付款'" @click="cancelOrder(item.id)">取消订单</button>
							<!-- <button style="position: absolute;margin-left: -2.2rem;" v-show="item.status=='待付款'" @click="payMoney(item.id)">确认付款</button> -->
							<button style="position: absolute;margin-left: -2.2rem;" v-show="item.status=='待收货'" @click="wait(item.id)">确认收货</button>
							共{{item.number}}件
							<span>小计：</span>
							<i>￥{{item.allmoney}}</i>
						</p>
	...
	export default {
		...
		methods: {
			...
			getExperienceOrder() {
				...
				mui.ajax(config.host + "/mallOrder", {
			...
			deleteOrder: function(id) {
				...
				mui.confirm("您确定删除订单吗?", "", ["取消", "确认"], function(e) {
					if (e.index != 0) {
						mui.ajax(config.host + "/qDelOrder", {
							dataType: "json", //服务器返回json格式数据
							headers: {
								token: token
							},
			...
			btnpassword(text) {
				...
				if (this.index == 5) {
					...
					let _this = this;
					let token = localStorage.getItem("token");
					mui.ajax(config.host + "/exsubmitorder", {
						data: {
							id: _this.orderid,
			...
			wait: function(id) {
				let _this = this;
				let token = localStorage.getItem("token");
				mui.ajax(config.host + "/receiptorder", {
					dataType: "json", //服务器返回json格式数据
					headers: {
						token: token
					},
